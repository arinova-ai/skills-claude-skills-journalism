...
upstream `secure-auth` audit snapshot remains recoverable from the parent Git
history and its documented checksum, but is deliberately absent from the
import-closure tree because the production scanner evaluates every source file.
